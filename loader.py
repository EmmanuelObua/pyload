...
logging.basicConfig(filename='pyload.log', level=logging.INFO,
...
def get_file_info(folder_name):
    ...
    info = file_types_info.get(
        folder_name, {'column_names': None, 'table_name': None})
...
def process_zip_file(zip_file_path, temp_dir, folder, ext='.zip'):
    ...
    try:
        ...
        if ext == '.zip':
            ...
            output_file = os.path.join(temp_dir, os.path.splitext(
                os.path.basename(zip_file_path))[0])
...
def load(env):
    ...
    try:
        zipfiles_dir = os.path.abspath(
            os.path.join(os.path.dirname(__file__), 'cdrs'))
        loaded_dir = os.path.abspath(os.path.join(
            os.path.dirname(__file__), 'cdrs/loaded'))
        ...
        for folder in folders:
            ...
            folder_files = os.path.abspath(os.path.join(
                os.path.dirname(__file__), f'cdrs/{folder}'))

            for file_name in os.listdir(folder_files):

                random_string_name = generate_random_string()
                cleaned_file_path = os.path.join(
                    temp_dir, f'{random_string_name}_cleaned_file.csv')

                zip_file_path = os.path.join(folder_files, file_name)
                if file_name.endswith(".zip"):
                    unzipped_file_name, table_name, column_names = process_zip_file(
                        zip_file_path, temp_dir, folder, ext='.zip')
                elif file_name.endswith(".gz"):
                    unzipped_file_name, table_name, column_names = process_zip_file(
                        zip_file_path, temp_dir, folder, ext='.gz')
                else:
                    return
                ...
                command = f"mysql --local-infile -h {env['MYSQL_HOST']} -u {env['MYSQL_USER_NAME']} -p -D {env['MYSQL_DATABASE']} -e \"LOAD DATA LOCAL INFILE '{cleaned_file_path}' INTO TABLE {table_name} FIELDS TERMINATED BY ',' LINES TERMINATED BY '\\n';\""
